...
namespace NetCoreDispatcher.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class MsmqController : ControllerBase
    {
        private readonly ILogger<MsmqController> _logger;

        public MsmqController(ILogger<MsmqController> logger)
        {
            _logger = logger;
        }
        ...
        [HttpPost]
        // api Send
        public dynamic Send([FromBody] _Pregunta question)
        {
            ...
            try
            {
                ...
                message.Body = question.url;
                message.Label = question.label;
                _messageQueue.Send(message);

                messageResponses.Error = false;
                messageResponses.Status = "El mensaje se envío correctamente";
                listMessages.Add(new InnerMessages() { body = question.url, label = question.label, id = "" });
